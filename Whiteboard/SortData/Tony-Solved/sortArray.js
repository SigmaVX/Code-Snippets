...
sortMethod = (event) => {
  ...
  var sortedArray = unsortedArr.sort(function(a,b){return a-b});
  document.getElementById("sortMethod").innerHTML = sortedArray.join(" ");
}
...
linearSort = () =>{
  ...
  for(i=1; i<unsortedArr.length; i++){
    if(unsortedArr[i-1]>unsortedArr[i]){
      tempNum = unsortedArr[i];
      unsortedArr[i] = unsortedArr[i-1];
      unsortedArr[i-1] = tempNum;
    }
  }
  document.getElementById("linearMethod").innerHTML = unsortedArr.join(" ");
}
...
const printQuickSort = () => {
  document.getElementById("quickSortMethod").innerHTML = quickSort(unsortedArr).join(" ");
}

// Event Listeners
document.getElementById("go").addEventListener("click", sortMethod);
document.getElementById("go").addEventListener("click", linearSort);
document.getElementById("go").addEventListener("click", printQuickSort);
